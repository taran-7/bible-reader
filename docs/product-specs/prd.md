...
- Реалізація: allowlist доменів у конфігурації (передається у веб-пошук як дозволені домени) плюс blocklist православних і католицьких доменів як запасна перевірка. Список доменів формуємо й переглядаємо вручну ([illustration-sources.md](illustration-sources.md)); `BibleCore` відкидає будь-яку історію, у якої хоч одне джерело поза allowlist.
