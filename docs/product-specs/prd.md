...
### v1.4 Ілюстрації
**6.17 Кнопка «Пошук ілюстрацій».** Поруч із кнопкою копіювання (6.3) на виділенні одного чи кількох віршів з'являється друга напівпрозора кнопка «Пошук ілюстрацій». Клік відкриває окреме вікно з ілюстраціями до цієї біблійної події чи людей (картини, гравюри, історичні зображення): до 7 штук, найрелевантніші першими.
Під капотом: з тексту віршів і посилання (`Ин 3:16-18`) будуємо промпт; LLM визначає подію, людей і місце й повертає кілька пошукових запитів (англійською, бо так більше результатів). Запити йдуть у пошук зображень в інтернеті (Google або аналог), результати зводимо без дублікатів, відбираємо 7.
Вікно: сітка мініатюр; для кожної назва, автор/джерело, посилання на сторінку-першоджерело. Клік відкриває зображення в повному розмірі. Під час пошуку індикатор завантаження; без мережі чи при помилці зрозуміле повідомлення «Не вдалося знайти ілюстрації» і кнопка «Повторити».
Логіка (промпт, розбір відповіді, дедуплікація, відбір) живе в `BibleCore` за протоколами `IllustrationPromptBuilder` / `ImageSearchProvider`, щоб тестувати без мережі на фейках. Ключі API зберігаються в Keychain, вводяться в Settings; без ключа кнопка неактивна з підказкою.
Критерій: без виділення кнопки немає; на Быт 22:1-13 (жертвопринесення Ісаака) у вікні ≤ 7 зображень, щонайменше 5 стосуються сюжету; у вікні не більше 7 зображень і жодного дубліката; жодних мережевих запитів до кліку.
...
- Офлайн; жодної телеметрії. Мережа лише для пошуку ілюстрацій (6.17) і тільки за явним кліком; у запит іде лише текст виділених віршів.
...
- **Ілюстрації (6.17): API пошуку.** Треба обрати провайдера пошуку зображень (Google Custom Search, Bing, Brave, SerpAPI тощо) і LLM; перевірити актуальні умови, ціну і ліміти, бо API Google для пошуку зображень міг змінитися. Альтернатива без ключів пошуку: Wikimedia Commons (відкриті ліцензії).
- **Ілюстрації: права на зображення.** Показуємо мініатюри з посиланням на першоджерело, нічого не зберігаємо в додатку; для експорту чи копіювання зображень потрібна ліцензія.
- **Ілюстрації: релевантність.** Пошук може повертати сучасні фото чи нерелевантні картинки; потрібні фільтри (тип «painting/engraving», стоп-слова) і, можливо, повторна оцінка LLM.
