...
- Обмеження API веб-пошуку: до 64 доменів у списку, піддомени покриваються автоматично; `allowed_domains` і `blocked_domains` в одному запиті разом не передаються, тож у запит іде allowlist, а blocklist перевіряє `BibleCore`.
