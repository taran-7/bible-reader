...
- [x] 5.3 `Reference.format` і `Quote.format`; тести `QuoteTests.testSingleVerseSynodal` (`(Ин. 3:16)`), `testRangeKjv` (`(John 3:16-18)`), `testJoinsTextsWithSpace`
...
- [ ] 7.6 Екран помилки бази; ручна перевірка з перейменованим `bible.sqlite`
...
- [ ] 8.2 `swift test` зелений, прохід рецензента по diff; перевірка: вивід `swift test` без падінь
